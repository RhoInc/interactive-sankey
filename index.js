(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.interactiveSankey = factory(global.webCharts));
    ...
    function getXAxisOrder() {
      var _this = this;

      var xAxisSet = new Set(this.raw_data.map(function (d) {
        return d[_this.config.node_col];
      }));

      var xAxisArray = _toConsumableArray(xAxisSet.values());

      var xAxisOrder = xAxisArray.every(function (value) {
        return !isNaN(parseInt(value));
      }) ? xAxisArray.sort(function (a, b) {
    ...
    function getLegendOrder() {
      ...
      var states = _toConsumableArray(new Set(this.raw_data.map(function (d) {
        return d[_this.config.link_col];
      })).values()).sort(d3.ascending);

      var legendOrder = this.config.legend.order;
      if (this.config.legend.order === undefined || this.config.legend.order.constructor !== Array) legendOrder = states;else {
    ...
    function drawLinks(bars1, bars2, linkClass) {
      ...
      var chartObject = this; //Merge adjacent bar groups by [ config.y.column ].
    ...
    function onResize() {
      var chart = this; //Reset displays.

      this.wrap.selectAll('path.link').remove();
      this.wrap.selectAll('.barAnnotation').remove(); //Update legend to represent categories represented in chart.
      ...
      var makeSelector = function makeSelector(value) {
        return value.replace(/[^0-9_a-z]/gi, '-');
      };

      var getNodeId = function getNodeId(x, stratum) {
        return "#node-".concat(makeSelector(x), "-").concat(makeSelector(stratum));
      ...
      barGroups.each(function (barGroup, i) {
        console.log(barGroup); //Annotate bars and modify tooltips.
        ...
        d3.select(this).selectAll('rect.wc-data-mark').each(function (d) {
          ...
          d3.select(bar.node().parentNode).append('text').datum([{
            ...
            id: function id(di) {
              return getNodeId(d.values.x, d.key).replace('#', '');
            },
            x: function x(di) {
      ...
      bars.style('cursor', 'pointer').on('click', function (d) {
        ...
        chart.wrap.selectAll('.selectedLink').remove(); //Capture [ settings.id_col ] values represented by selected bar.
    ...
    function interactiveSankey(element, settings) {
      ...
      var mergedSettings = Object.assign({}, defaultSettings, settings); //Sync settings with data mappings.

      var syncedSettings = syncSettings(mergedSettings); //Create chart.
