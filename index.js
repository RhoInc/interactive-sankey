(function (global, factory) {
    ...
    function onInit() {
        ...
        this.raw_data = this.raw_data.sort(function (a, b) {
            return a[_this.config.node_col] < b[_this.config.node_col]
                ? -1
                : a[_this.config.node_col] > b[_this.config.node_col]
                ? 1
                : 0;
        }); // Define order of states.

        var states = _toConsumableArray(
            new Set(
        ...
        if (
            this.config.legend.order === undefined ||
            this.config.legend.order.constructor !== Array
        )
            this.config.legend.order = states;
        else {
            states.forEach(function (state) {
                if (_this.config.legend.order.includes(state) === false)
                    _this.config.legend.order.push(state);
            });
        }
    }
    ...
    function drawLinks(bars1, bars2, linkClass) {
        ...
        nestedLinkData.sort(function (a, b) {
            return (
                _this.config.legend.order.indexOf(a.key) - _this.config.legend.order.indexOf(b.key)
            );
        ...
        nestedLinkData.forEach(function (d, i) {
            ...
            d.values.sort(function (a, b) {
                return (
                    _this.config.legend.order.indexOf(a.key) -
                    _this.config.legend.order.indexOf(b.key)
                );
            });
    ...
    function onResize() {
        ...
        this.wrap
            ...
            .filter(function () {
                return (
                    currentLinks.indexOf(
                        d3.select(this).select('.legend-label')[0][0].textContent
                    ) === -1
                );
            })
            .remove();
      ...
      \-------------------------------------------------------------------------------------------**/
        ...
        bars.style('cursor', 'pointer').on('click', function (d) {
            ...
            var selectedData = d3
                ...
                .entries(
                    chart.raw_data.filter(function (d) {
            ...
            chart.svg
                ...
                .text(function (d) {
                    return (
                        d.values.y + ' (' + d3.format('%')(d.values.y / selectedIDs.length) + ')'
                    );
                }); //Draw links.
