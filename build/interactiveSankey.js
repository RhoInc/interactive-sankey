            ...
            if (i < nBarGroups) {
                    ...
                    bars2.each(function (bar2, i2) {
                        bar1.values.raw.forEach(function (id1) {
                            bar2.values.raw.forEach(function (id2) {
                                if (id1[context.config.y.column] === id2[context.config.y.column]) linkData.push({ id: id1[context.config.y.column],
                ...
                collapsedLinkData.forEach((d, i) => {
                    var path = [{ split1: d.split1,
                        ...
                        y0: context.y(d.y01),
                        y1: context.y(d.y11) }, { x: d.x2,
                        y0: context.y(d.y02),
                        y1: context.y(d.y12) }];
                    context.svg.append('path').datum(path).attr({ 'd': pathDrawer,
                        'class': 'link' }).style({ 'fill': () => context.colorScale(d.split1),
                        'fill-opacity': .5,
